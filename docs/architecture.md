...
1. Establish a strict Infrastructure Status v1 boundary.
2. Add publication configuration and validation without exposing a mutation API publicly.
...
The development control plane binds to `127.0.0.1:9080` by default and exposes only liveness and sanitized status. No route mutation, certificate mutation, credential management, or Caddy admin proxy is implemented in this slice.
