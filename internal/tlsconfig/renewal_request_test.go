...
func TestBuildRenewalRequestRequiresEligibilityAndMonotonicTime(t *testing.T) {
	...
	request, err := BuildRenewalRequest(evidence, now.Add(time.Minute))
	if err != nil { t.Fatal(err) }
	if err := request.Validate(); err != nil { t.Fatal(err) }
	if request.ProfileID != "primary" || request.CurrentSerial != "42" { t.Fatalf("unexpected request: %+v", request) }

	if _, err := BuildRenewalRequest(evidence, now.Add(-time.Minute)); err == nil {
...
func TestBuildRenewalRequestRejectsCertificateOutsideWindow(t *testing.T) {
	...
	if _, err := BuildRenewalRequest(evidence, now); err == nil {
