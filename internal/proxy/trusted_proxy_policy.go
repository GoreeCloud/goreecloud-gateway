...
// NewTrustedProxyPolicy builds a fail-closed trusted-proxy policy from the same
// normalized configuration validation used by the Gateway config loader.
func NewTrustedProxyPolicy(entries []string) (TrustedProxyPolicy, error) {
	prefixes, err := config.ParseTrustedProxyPrefixes(entries)
	if err != nil {
		return TrustedProxyPolicy{}, err
	}
	return TrustedProxyPolicy{prefixes: prefixes}, nil
}
