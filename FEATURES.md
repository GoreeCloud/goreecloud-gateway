...
- approved operator execution/human TOS review using the now-wired account-registration tool, approved wrapping-key provisioning/escrow and target recovery rehearsal, transactional CA-side account-key rollover/activation using the now-prepared bundle boundary, plus live CA/Porkbun issuance and renewal acceptance over the established RFC 8555 DNS-01 issuer;
